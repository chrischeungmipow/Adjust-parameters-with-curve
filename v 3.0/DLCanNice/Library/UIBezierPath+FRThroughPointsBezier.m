//
//  UIBezierPath+FRThroughPointsBezier.m
//  fristDome
//
...
@implementation UIBezierPath (FRThroughPointsBezier)

- (void)setContractionFactor:(CGFloat)contractionFactor {
  objc_setAssociatedObject(self, @selector(contractionFactor),
                           @(contractionFactor),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGFloat)contractionFactor {
  id contractionFactorAssociatedObject =
      objc_getAssociatedObject(self, @selector(contractionFactor));
...
- (void)addBezierThroughPoints:(NSArray *)pointArray {
  for (int i = 0; i < pointArray.count - 1; i++) {
    CGPoint firstPoint = [[pointArray objectAtIndex:i] CGPointValue];
    CGPoint secondPoint = [[pointArray objectAtIndex:i + 1] CGPointValue];
    if (i == 0)
      [self moveToPoint:firstPoint];
      CGFloat controlerX = (firstPoint.x)?firstPoint.x:5;
    [self addCurveToPoint:secondPoint
            controlPoint1:CGPointMake((secondPoint.x - firstPoint.x) / 2 +
                                          self.contractionFactor * controlerX,
                                      firstPoint.y)
            controlPoint2:CGPointMake((secondPoint.x - firstPoint.x) / 2 +
                                         self.contractionFactor * controlerX,
                                      secondPoint.y)];
  }
...
//      [self addQuadCurveToPoint:point1
//                   controlPoint:ControlPointForTheBezierCanThrough3Point(
//                                    self.currentPoint, point0, point1)];
//    } break;
//    default:
//      break;
//    }
...
//
//      centerPointDistance =
//          DistanceBetweenPoint(previousCenterPoint, centerPoint);
//
//      obliqueAngle =
//          ObliqueAngleOfStraightThrough(centerPoint, previousCenterPoint);
//
//      previousControlPoint2 =
//          CGPointMake(previousPoint.x -
//                          0.5 * self.contractionFactor * centerPointDistance *
//                              cos(obliqueAngle),
//                      previousPoint.y -
//                          0.5 * self.contractionFactor * centerPointDistance *
//                              sin(obliqueAngle));
...
//    if (i == 1) {
//
//      [self addQuadCurveToPoint:previousPoint
//                   controlPoint:previousControlPoint2];
//    } else if (i > 1 && i < pointArray.count - 1) {
//
//      [self addCurveToPoint:previousPoint
//              controlPoint1:previousControlPoint1
//              controlPoint2:previousControlPoint2];
...
//
////        if(!self.isDrawPoint){
////            for(int i=0; i<[[pointArray objectAtIndex:0] count]; i++){
////                CGContextRef ctx = UIGraphicsGetCurrentContext();
////                CGPoint point = [[[pointArray objectAtIndex:0]
///objectAtIndex:i] CGPointValue];
////                CGContextFillEllipseInRect(ctx, CGRectMake(point.x-4,
///point.y-4, 8, 8));
...
CGFloat ObliqueAngleOfStraightThrough(CGPoint point1,
                                      CGPoint point2) //  [-π/2, 3π/2)
{
...
CGPoint ControlPointForTheBezierCanThrough3Point(CGPoint point1, CGPoint point2,
                                                 CGPoint point3) {
  return CGPointMake(2 * point2.x - (point1.x + point3.x) / 2,
                     2 * point2.y - (point1.y + point3.y) / 2);
}
